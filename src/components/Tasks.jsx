  ...
  function Tasks({tasklist, addTask, checkVal, setCheckVal}) {
    return (
      <div className="wrapper overflow-hidden box-content flex w-[50%] max-h-[95%] rounded-xl">
        ...
        {tasklist.map((item, index)=>(
          <Task key={index} checkValue={checkVal} setCheckValue={setCheckVal} tasklist={tasklist} addTask={addTask} item={item} index={index}/>
        ))}
      </div>
      </div>
    )
